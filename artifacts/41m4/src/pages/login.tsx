    ...
    <div className="min-h-screen bg-background text-foreground font-mono flex flex-col">
      <nav className="border-b border-border/50 p-4 bg-background/90 backdrop-blur">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" onClick={() => setLocation("/register")} className="text-muted-foreground hover:text-primary rounded-none">[REGISTER]</Button>
        </div>
      </nav>
        ...
        <div className="w-full max-w-md">
          <div className="border border-primary shadow-[0_0_30px_rgba(0,255,65,0.15)] bg-card">
            <div className="bg-primary/20 border-b border-primary p-3 flex items-center gap-2">
              ...
              <span className="ml-3 text-xs text-primary font-bold tracking-widest">auth@41m4:~/login</span>
            </div>

            <div className="p-8 space-y-6">
              <div className="flex items-center gap-3">
                <Terminal className="w-6 h-6 text-primary" />
                <h1 className="text-xl font-bold tracking-widest text-primary uppercase">Authenticate</h1>
              </div>

              <div className="text-muted-foreground text-xs space-y-1 border border-border/40 bg-background/30 p-3">
                <div><span className="text-primary">$</span> Establish secure session</div>
                <div><span className="text-primary">$</span> Credentials required → proceed</div>
              </div>
